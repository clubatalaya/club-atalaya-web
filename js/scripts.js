  ...
  fetch(WORKER_URL + '/api/carteles')
    ...
    .then(function(cfg) {
      if (cfg.oratorio && cfg.oratorio.key) {
        var el = document.getElementById('cartelOratorio');
        if (el) el.src = WORKER_URL + '/api/archivo/' + encodeURIComponent(cfg.oratorio.key);
      }
      if (cfg.centrojuvenil && cfg.centrojuvenil.key) {
        var el2 = document.getElementById('cartelCentroJuvenil');
        if (el2) el2.src = WORKER_URL + '/api/archivo/' + encodeURIComponent(cfg.centrojuvenil.key);
      }
    }).catch(function() {});
}
    ...
    .then(function(lista) {
      ...
      if (grid && destacados.length) {
        ...
        destacados.forEach(function(p) {
          ...
          (function(u, t) {
            btn.onclick = function(e) { e.stopPropagation(); abrirPDF(u, t); };
      ...
      if (listaEl && enLista.length) {
        ...
        enLista.forEach(function(p) {
          ...
          li.style.cssText = 'padding:8px 0;border-bottom:1px solid #f0f0f0;display:flex;align-items:center;gap:10px';
  ...
  fetch(WORKER_URL + '/api/redes')
    ...
    .then(function(redes) {
      ...
      var SVG = {
        ...
        youtube:'<svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><path d="M23.495 6.205a3.007 3.007 0 00-2.088-2.088c-1.87-.501-9.396-.501-9.396-.501s-7.507-.01-9.396.501A3.007 3.007 0 00.527 6.205a31.247 31.247 0 00-.522 5.805 31.247 31.247 0 00.522 5.783 3.007 3.007 0 002.088 2.088c1.868.502 9.396.502 9.396.502s7.506 0 9.396-.502a3.007 3.007 0 002.088-2.088 31.247 31.247 0 00.5-5.783 31.247 31.247 0 00-.5-5.805zM9.609 15.601V8.408l6.264 3.602z"/></svg>',
      ...
      redes.forEach(function(r) {
        ...
        a.className = 'red-social-icono';
        a.setAttribute('aria-label', r.nombre);
        var circulo = document.createElement('div');
        circulo.className = 'rsi-circulo rsi-' + (r.icono||'otro');
        circulo.innerHTML = (SVG[r.icono]||SVG.otro) + '<span class="rsi-etiqueta">' + r.nombre + '</span>';
        var nombre = document.createElement('span');
        nombre.className = 'rsi-nombre'; nombre.textContent = r.nombre;
        a.appendChild(circulo); a.appendChild(nombre);
        cont.appendChild(a);
      });
    }).catch(function() {});
}
